...
namespace Turbine.Consumer.AspenTech73
{
    ...
    /// AspenSinterConsumer
    /// </summary>
    public class AspenSinterConsumer : Turbine.Consumer.AspenTech73.SinterConsumerRun
    {
        ...
        {
            Debug.WriteLine(String.Format("MonitorTerminate '{0}'", applicationName), GetType().Name);
            if ("ACM".Equals(applicationName)) 
            {
                foreach (Process p in Process.GetProcessesByName("AspenModeler")) {
                    Debug.WriteLine("MonitorTerminate: Kill AspenModeler processId=" + p.Id, GetType().Name);
                    p.Kill();
                }
            }
            //base.MonitorTerminate();
        }*/
