...
$cmd_str = "SELECT Id,ConsumerId FROM ( SELECT *, ROW_NUMBER() OVER (ORDER BY Id) as row FROM dbo.Jobs  WHERE state='running') a WHERE row > 0 and row <= 1000"
...
while ($reader.Read())
{
	...
	$cmd_str = "UPDATE dbo.Jobs SET state='submit',Setup='',Running='' WHERE Id='{0}'" -f $reader.GetValue(0)
	echo $cmd_str
	...
	$cmd_str = "INSERT INTO dbo.Messages (Id,Value,JobId) VALUES ('{0}','{1}','{2}')" -f ([guid]::NewGuid(), "JobSweeper Reset From Running, detected Consumer down", $reader.GetValue(0))
	echo $cmd_str
	$update_cmd = new-object system.data.sqlclient.sqlcommand($cmd_str, $conn)
	$update_reader = $update_cmd.ExecuteReader();
