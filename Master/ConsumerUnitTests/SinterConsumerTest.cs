...
namespace ConsumerUnitTests
{
    ...
    public class SinterExecutionTest
    {
        ...
        public void SinterACMTest()
        {
            ...
            Directory.CreateDirectory("testACM");
            sim.workingDir = Path.Combine(cwd, "testACM");
            string configFileName = Path.Combine(sim.workingDir, Properties.Settings.Default.ACMFilename);
            string config = System.Text.Encoding.ASCII.GetString(buffer);

            // Retrieve backup filename from sinter config file
            String filename = sinter_HelperFunctions.getBackupFilename(config);
            ...
            byte[] backupData = File.ReadAllBytes(Path.Combine(Properties.Settings.Default.ACMDir, filename));
        ...
        public void SinterAspenPlusTest()
        {
            ...
            string cwd = Directory.GetCurrentDirectory();
            sim.workingDir = Path.Combine(cwd, "test");
            string configFileName = Path.Combine(sim.workingDir, Properties.Settings.Default.APFilename);
            string config = System.Text.Encoding.ASCII.GetString(buffer);
            ...
            byte[] backupData = File.ReadAllBytes(Path.Combine(Properties.Settings.Default.APDir, filename));
            var backupFileName = Path.Combine(sim.workingDir, filename);
            File.WriteAllBytes(backupFileName, backupData);
            IDictionary<string, Object> myDict = null;
