...
namespace ConsumerUnitTests
{
    ...
    public class SinterInputTests
    {
        ...
        /**
         * Tests the ability to pull the filename out of a version 0.2 formatted config file.
         **/
        [TestMethod]
        public void BackupFilenameTest2()
        {
            string ct = File.ReadAllText(Properties.Settings.Default.jsonConfiguration);
            String filename = sinter_HelperFunctions.getBackupFilename(ct);
            String shortFilename = System.IO.Path.GetFileNameWithoutExtension(filename);
            Assert.IsTrue(shortFilename.CompareTo("Flash_Example") == 0);
        }
