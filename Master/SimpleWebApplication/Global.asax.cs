...
namespace SimpleWebApplication
{
    public class Global : HttpApplication
    {
        ...
        {
            Debug.WriteLine("Application_Start", "Global");
            RouteTable.Routes.Add(new ServiceRoute("application", new WebServiceHostFactory(), typeof(ApplicationResource)));
            RouteTable.Routes.Add(new ServiceRoute("simulation", new WebServiceHostFactory(), typeof(SimulationResource)));
            RouteTable.Routes.Add(new ServiceRoute("job", new WebServiceHostFactory(), typeof(JobResource)));
            RouteTable.Routes.Add(new ServiceRoute("session", new WebServiceHostFactory(), typeof(SessionResource)));
            // TODO: Create ConsumerREsource with EF6 Backend
            //RouteTable.Routes.Add(new ServiceRoute("consumer", new WebServiceHostFactory(), typeof(ConsumerResource)));
        }

        void Application_AuthenticateRequest(Object sender, EventArgs e)
        {
            Debug.WriteLine(String.Format("Application_AuthenticateRequest: {0}", 
                HttpContext.Current.Request.UserHostAddress), "Global");
            if (! AuthenticateUser())
            {
